...
import { runAnalysis } from '../services/pythonService.js'; // Keep .js extension for ESM compatibility
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        if (!validHorizons.includes(horizon.toLowerCase())) {
            return res.status(400).json({
                status: 'error',
                message: `Invalid horizon. Must be one of: ${validHorizons.join(', ')}`
            });
        }
        if (!validRiskProfiles.includes(risk_profile.toLowerCase())) {
            return res.status(400).json({
                status: 'error',
        ...
        // Run analysis
        const result = await runAnalysis(ticker.toUpperCase(), horizon.toLowerCase(), risk_profile.toLowerCase());
        // Format response
        res.json({
            status: 'success',
            data: result,
...
router.post('/analyze/stream', async (req, res) => {
    try {
        ...
        runAnalysis(ticker.toUpperCase(), horizon.toLowerCase(), risk_profile.toLowerCase(), sendProgress)
            .then((result) => {
            clearInterval(keepAliveInterval);
